...
<form method="post" class="mt-4">
    ...
    <select class="form-select" name="segmentedImage" id="segmentedImageSelect">
        <?php
        $imageId = $_GET['id'];
        $sqlSegmentedImages = "SELECT si_ID, si_nome FROM si_segmented_images WHERE si_id_ImmagineOriginale = $imageId";
        ...
        if ($segmentedImagesResult->num_rows > 0) {
            while ($segmentedRow = $segmentedImagesResult->fetch_assoc()) {
                echo '<option value="' . $segmentedRow["si_nome"] . '"data-id="' . $segmentedRow["si_ID"] . '">' . $segmentedRow["si_nome"] . '</option>';
            }
        } else {
...
<script>
    ...
    segmentedImage.onload = function() {
        ...
        segmentedImage.style.width = '500px';
    ...
    const segmentedImageSelect = document.getElementById('segmentedImageSelect');

    // Se c'è solo un'opzione nella select, forza l'evento change
    if (segmentedImageSelect.options.length === 1) {
        segmentedImageSelect.selectedIndex = 0;  
        segmentedImageSelect.dispatchEvent(new Event('change'));  // Forza l'evento change
    }
});
</script>
<?php
